package com.lucasdnd.loneblock.gameplay;
...
import com.lucasdnd.loneblock.gameplay.items.Usable;
...
import com.lucasdnd.loneblock.gameplay.world.pathfinder.Mover;
...
public class Player implements Mover {
	...
	public void update() {
		...
		World world = ((LoneBlock)Gdx.app.getApplicationListener()).getWorld();
		...
		if (statusDecrease % statusDecreaseLimit == 0) {
			if (((LoneBlock)Gdx.app.getApplicationListener()).getTimeController().isNight()) {
				cold -= 5;
			} else {
				cold--;
			}
	...
	public void render(ShapeRenderer sr) {
		...
		sr.setColor(color);
		sr.rect(x * LoneBlock.blockSize, y * LoneBlock.blockSize, LoneBlock.blockSize, LoneBlock.blockSize);
		float directionOffsetX = 0f;
		float directionOffsetY = 0f;
		switch (direction) {
		case UP:
			directionOffsetX = LoneBlock.blockSize / 2f - LoneBlock.blockSize / 4f;
			directionOffsetY = LoneBlock.blockSize + LoneBlock.blockSize / 4f;
			sr.rect(x * LoneBlock.blockSize + directionOffsetX, y * LoneBlock.blockSize + directionOffsetY, LoneBlock.blockSize / 2f, LoneBlock.blockSize / 2f);
			break;
		case LEFT:
			directionOffsetX = -LoneBlock.blockSize / 2f - LoneBlock.blockSize / 4f;
			directionOffsetY = LoneBlock.blockSize / 2f - LoneBlock.blockSize / 4f;
			sr.rect(x * LoneBlock.blockSize + directionOffsetX, y * LoneBlock.blockSize + directionOffsetY, LoneBlock.blockSize / 2f, LoneBlock.blockSize / 2f);
			break;
		case DOWN:
			directionOffsetX = LoneBlock.blockSize / 2f - LoneBlock.blockSize / 4f;
			...
			break;
		case RIGHT:
			directionOffsetX = LoneBlock.blockSize + LoneBlock.blockSize / 4f;
			directionOffsetY = LoneBlock.blockSize / 2f - LoneBlock.blockSize / 4f;
			sr.rect(x * LoneBlock.blockSize + directionOffsetX, y * LoneBlock.blockSize + directionOffsetY, LoneBlock.blockSize / 2f, LoneBlock.blockSize / 2f);
			break;
		default:
