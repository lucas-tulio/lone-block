...
import com.lucasdnd.loneblock.gameplay.world.pathfinder.AStarPathFinder;
import com.lucasdnd.loneblock.gameplay.world.pathfinder.PathFindingContext;
import com.lucasdnd.loneblock.gameplay.world.pathfinder.TileBasedMap;
...
public class World implements Disposer, TileBasedMap {
	...
	public void render(ShapeRenderer sr) {
		...
		sr.rect(0f, 0f, size * LoneBlock.blockSize, size * LoneBlock.blockSize);
		...
		float visibleBlocksX = gameViewWidth / LoneBlock.blockSize;
		float visibleBlocksY = Gdx.graphics.getHeight() / LoneBlock.blockSize;
		...
		for (int i = minRenderX; i < maxRenderX; i++) {
			for (int j = minRenderY; j < maxRenderY; j++) {
				...
				if (mapObject != null) {
					mapObject.render(sr, i * LoneBlock.blockSize, j * LoneBlock.blockSize);
				}
			}
		}
