...
import com.lucasdnd.loneblock.ui.ButtonClickListener;
import com.lucasdnd.loneblock.ui.DialogBox;
import com.lucasdnd.loneblock.ui.SideBar;
import com.lucasdnd.loneblock.ui.Tooltip;

public class LoneBlock extends ApplicationAdapter {
	
	// General stuff
	public final static String GAME_NAME = "Lone Block";
	public final static String VERSION = "v0.6.0";
	...
	private static void updateZoomLevelDisplay() {
		LoneBlock.zoomLevelDisplay = (int)(Math.log(blockSize) / Math.log(2));
	}
	...
	private void handleInput() {
		...
		if (paused == false) {
			...
			if (Gdx.input.isKeyJustPressed(Keys.EQUALS)) {
				...
				LoneBlock.updateZoomLevelDisplay();
	...
	private void update() {
		...
		if (player.isDead() == false) {
			...
			if (paused == false) {
				camera.position.set(player.getX() * LoneBlock.blockSize + (SideBar.SIDEBAR_WIDTH * 0.5f), player.getY() * LoneBlock.blockSize, 0f);
				camera.update();
	...
	public void render () {
		...
		if (debug && waiting == false) {
			...
			Resources.get().whiteFont.draw(fontBatch, "zoom: " + LoneBlock.zoomLevelDisplay, 0f, Gdx.graphics.getHeight() - 200f);
